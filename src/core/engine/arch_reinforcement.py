...
class ArchReinforcementManager:
    ...
    def check_bendability(profile_name, radius_cm, steel_grade='S235'):
        ...
        # Verifica limiti calandratura
        if r_h < 10:
            result['bendable'] = False
            result['method'] = 'Non calandrabile - raggio troppo stretto'
            result['warnings'].append(f"Rapporto r/h = {r_h:.1f} < 10 - Rischio rottura")
            
        elif r_h < 15:
            result['bendable'] = True
            result['method'] = 'Calandratura a caldo obbligatoria'
            result['warnings'].append(f"Rapporto r/h = {r_h:.1f} < 15 - Solo calandratura a caldo")
            ...
            result['warnings'].append(f"Rapporto r/h = {r_h:.1f} < 30 - Preferibile a caldo")
        ...
        E = 210000  # MPa - modulo elastico acciaio
...
class BendingVerificationDialog(QDialog):
    ...
    def calculate(self):
        ...
        # Tensioni residue
        fy = {'S235': 235, 'S275': 275, 'S355': 355, 'S450': 450}.get(self.steel_grade, 235)
